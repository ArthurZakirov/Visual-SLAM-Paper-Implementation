...
private:
	Voxel();
	Voxel(double sdf_, double weight_): sdf(sdf_), weight(weight_){}
public:
	double sdf;
	double weight;
};
...
public:
	DiscreteSurface(Vector3d min_, Vector3d max_, uint resolution_): min(min_), max(max_), resolution(resolution_) {
		this->grid_size = 1.0 / (resolution - 1);
	}
	...
	Voxel* operator() (uint x_, uint y_, uint z_)
	{
		Voxel* voxel = &voxels[getPosFromTuple(x_, y_, z_)];
		return voxel;
	}

	Vector3d get_world_position(uint x_, uint y_, uint z_)
	{
		Vector3d coord(0, 0, 0);
		coord[0] = min[0] + (max[0] - min[0]) * (double(x_) * this->grid_size);
		coord[1] = min[1] + (max[1] - min[1]) * (double(y_) * this->grid_size);
		coord[2] = min[2] + (max[2] - min[2]) * (double(z_) * this->grid_size);
		return coord;
	}
